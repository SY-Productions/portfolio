...
const Cursor3D = () => {
  ...
  // Colors from the SVG
  const colors = {
    primary: "#7B2CBF", // Main green
    secondary: "#277F2A", // Darker green
    highlight: "#7B2CBF", // Highlight green (with different opacity)
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      trailElement.style.transform =
        "translate(-50%, -50%) perspective(400px) translateZ(0)";
  ...
  useEffect(() => {
    if (cursorRef.current) {
      // Update main cursor
      cursorRef.current.style.transform = `translate(${position.x}px, ${
        position.y
      }px) perspective(400px) translateZ(0px) rotateX(${
        position.y / 20
      }deg) rotateY(${-position.x / 20}deg)`;

      // Update trail elements with delay
      trailElementsRef.current.forEach((el, index) => {
        setTimeout(() => {
          if (el) {
            el.style.transform = `translate(${position.x}px, ${
              position.y
            }px) perspective(400px) translateZ(${
              -10 * (index + 1)
            }px) rotateX(${position.y / 10}deg) rotateY(${
              -position.x / 10
            }deg)`;
  ...
  return (
    <div
      ...
      style={{
        ...
        transition:
          "opacity 0.3s ease, transform 0.2s cubic-bezier(0.22, 1, 0.36, 1)",
      }}
    >
      ...
      <div
        ...
        style={{
          ...
          height: "14px",
          background:
            "radial-gradient(circle, rgba(55, 177, 59, 0.6) 0%, rgba(55, 177, 59, 0) 70%)",
