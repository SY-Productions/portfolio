...
const JSON_TEMPLATE = `{
  "name": "نام شرکت / سمت",
  "nameEn": "Company / Position Name",
  "nameAr": "اسم الشركة / المنصب",
  "fromYear": 2022,
  "toYear": null,
...
export default function WorksAdminPage() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (i) =>
        ...
        (Array.isArray(i.technos) ? i.technos.join(" ") : i.technos)
          .toLowerCase()
          .includes(q),
    );
  }, [items, search]);
  ...
  function openEdit(item: Work) {
    setForm({
      ...
      nameEn: item.nameEn ?? "",
      nameAr: item.nameAr ?? "",
      technos: Array.isArray(item.technos)
        ? item.technos.join(",")
        : item.technos,
      fromDate: {
  ...
  function handleParseJson() {
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        nameEn: p.nameEn ?? "",
        nameAr: p.nameAr ?? "",
        technos: Array.isArray(p.technos)
          ? p.technos.join(",")
          : p.technos ?? "",
        url: p.url ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      nameEn: form.nameEn,
      nameAr: form.nameAr,
      technos: (form.technos as string)
        .split(",")
        .map((t) => t.trim())
        .filter(Boolean),
    ...
    const url = isAdding ? "/api/works" : `/api/works/${form.id}`;
    ...
    if (res.ok) {
      toast.success(isAdding ? "Work added!" : "Updated!");
      setModalMode(null);
      fetchItems();
    } else {
  ...
  return (
    <div className="p-8">
        ...
        <div className="border border-white/10 overflow-hidden">
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-12 text-white/30">
              {search ? "No results found." : "No work records yet."}
            </div>
          )}
        </div>
      )}
      ...
      {modalMode && (
        <AdminModal
          title={modalMode === "add" ? "Add Work" : "Edit Work"}
          ...
          {inputMode === "json" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <TechTagSelector
                  value={form.technos as string}
                  onChange={(v) => setForm((p) => ({ ...p, technos: v }))}
                />
              </div>
              ...
              <div>
                <label className={labelClass}>Company Picture</label>
                <ImageGalleryField
                  value={form.picture}
